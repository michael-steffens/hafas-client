...
const parseDaysOffset = (_: Record<string, unknown>, time: string) => {
	return time.length > 6
		? parseInt(time.slice(0, -6))
		: 0;
};

const parseDateTime = (ctx: Record<string, unknown>, date: string, time: string, tzOffset: number | null = null, timestamp = false) => {
