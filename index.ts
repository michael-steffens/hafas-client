...
const isNonEmptyString = (str: unknown): str is string => 'string' === typeof str && str.length > 0;

const validateLocation = (loc: unknown, name = 'location') => {
	if (!isObj(loc)) {
		throw new TypeError(name + ' must be an object.');
	}
	const locObj = loc as Record<string, unknown>;
	if (locObj.type !== 'location') {
		throw new TypeError('invalid location object.');
	} else if ('number' !== typeof locObj.latitude) {
		throw new TypeError(name + '.latitude must be a number.');
	} else if ('number' !== typeof locObj.longitude) {
		throw new TypeError(name + '.longitude must be a number.');
	}
};

const validateWhen = (when: unknown, name = 'when') => {
	if (Number.isNaN(Number(when))) {
		throw new TypeError(name + ' is invalid');
	}
};

const createClient = (profile: any, userAgent: string, opt: any = {}) => {
	...
	const _stationBoard = async (station: unknown, type: unknown, resultsField: string, parse: (ctx: unknown, res: unknown) => unknown, opt: Record<string, unknown> = {}) => {
		if (isObj(station)) {
			station = profile.formatStation((station as Record<string, unknown>).id);
		...
		opt.when = new Date(Number(opt.when) || Date.now());
	...
	const departures = async (station: unknown, opt: Record<string, unknown> = {}) => {
		return await _stationBoard(station, 'DEP', 'departures', profile.parseDeparture, opt);
	};
	const arrivals = async (station: unknown, opt: Record<string, unknown> = {}) => {
		return await _stationBoard(station, 'ARR', 'arrivals', profile.parseArrival, opt);
	};

	const journeys = async (from: unknown, to: unknown, opt: Record<string, unknown> = {}) => {
		...
		let journeysRef: string | null = null;
		...
		if (opt.departure !== undefined && opt.departure !== null) {
			when = new Date(Number(opt.departure));
			if (Number.isNaN(Number(when))) {
			...
			when = new Date(Number(opt.arrival));
			if (Number.isNaN(Number(when))) {
		...
		const filters: unknown[] = [
			profile.formatProductsFilter({profile}, opt.products || {}) as unknown,
		];
		if (
			opt.accessibility
			&& profile.filters
			&& profile.filters.accessibility
			&& profile.filters.accessibility[opt.accessibility as string]
		) {
			filters.push(profile.filters.accessibility[opt.accessibility as string]);
		}

		if (!['slow', 'normal', 'fast'].includes(opt.walkingSpeed as string)) {
			throw new Error('opt.walkingSpeed must be one of these values: "slow", "normal", "fast".');
		}
		const gisFltrL: unknown[] = [];
	...
	const refreshJourney = async (refreshToken: unknown, opt: Record<string, unknown> = {}) => {
	...
	const journeysFromTrip = async (fromTripId: unknown, previousStopover: unknown, to: unknown, opt: Record<string, unknown> = {}) => {
		...
		} else if (isObj(to) && ((to as Record<string, unknown>).type === 'station' || (to as Record<string, unknown>).type === 'stop')) {
			to = profile.formatStation((to as Record<string, unknown>).id);
		} else {
		...
		let prevStop = (previousStopover as Record<string, unknown>).stop;
		if (isObj(prevStop)) {
			prevStop = profile.formatStation((prevStop as Record<string, unknown>).id);
		...
		let depAtPrevStop: number | string = ((previousStopover as Record<string, unknown>).departure || (previousStopover as Record<string, unknown>).plannedDeparture) as (string | number);
			...
			// interchange station. We slice the leg accordingly.
				...
				return {
					...
					legs: [
						...
						sliceLeg(fromLeg, (previousStopover as Record<string, unknown>).stop, fromLeg.destination),
	...
	const locations = async (query: unknown, opt: Record<string, unknown> = {}) => {
	...
	const stop = async (stop: unknown, opt: Record<string, unknown> = {}) => {
		if ('object' === typeof stop) {
			stop = profile.formatStation((stop as Record<string, unknown>).id);
	...
	const trip = async (id: string, opt: Record<string, unknown> = {}) => {
	...
	const tripsByName = async (lineNameOrFahrtNr = '*', opt: Record<string, unknown> = {}) => {
		...
		if (opt.when !== null) {
			req.date = profile.formatDate(profile, new Date(Number(opt.when)));
			req.time = profile.formatTime(profile, new Date(Number(opt.when)));
		}
		// todo: fromWhen doesn't work yet, but untilWhen does
		if (opt.fromWhen !== null) {
			req.dateB = profile.formatDate(profile, new Date(Number(opt.fromWhen)));
			req.timeB = profile.formatTime(profile, new Date(Number(opt.fromWhen)));
		}
		if (opt.untilWhen !== null) {
			req.dateE = profile.formatDate(profile, new Date(Number(opt.untilWhen)));
			req.timeE = profile.formatTime(profile, new Date(Number(opt.untilWhen)));
		}
		const filter = (mode, type, value) => ({mode, type, value});
		const filtersArr = req.jnyFltrL as unknown[];
		if (opt.currentlyStoppingAt !== null) {
			...
			filtersArr.push(filter('INC', 'STATIONS', opt.currentlyStoppingAt));
		}
		if (opt.lineName !== null) {
			...
			filtersArr.push(filter('INC', 'LINE', opt.lineName));
		}
		if (opt.operatorNames !== null) {
			...
			filtersArr.push(filter('INC', 'OP', (opt.operatorNames as string[]).join(',')));
		}
		req.jnyFltrL = [...filtersArr, ...(opt.additionalFilters as unknown[])];
	...
	const reachableFrom = async (address: unknown, opt: Record<string, unknown> = {}) => {
		...
		if (opt.when !== undefined && opt.when !== null) {
			opt.when = new Date(Number(opt.when));
			if (Number.isNaN(Number(opt.when))) {
				throw new TypeError('opt.when is invalid');
			}
		}
		...
		const byDuration: Array<{duration: number; stations: unknown[]}> = [];
	...
	const remarks = async (opt: Record<string, unknown> = {}) => {
		opt = {
		...
		if (opt.from !== null) {
			opt.from = new Date(Number(opt.from));
			validateWhen(opt.from, 'opt.from');
		}
		if (opt.to !== null) {
			opt.to = new Date(Number(opt.to));
	...
	const lines = async (query: string, opt: Record<string, unknown> = {}) => {
		if (!isNonEmptyString(query)) {
			throw new TypeError('query must be a non-empty string.');
		}
	...
	const serverInfo = async (opt: Record<string, unknown> = {}) => {
		opt = {
